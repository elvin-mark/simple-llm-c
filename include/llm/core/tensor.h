...
Tensor *create_tensor2(
    char *shape_string); // Wrapper over create_tensor for easy initialization.
                         // shape string should be like "2 3 4"
...
int increase_indices(
    int dim, int *indices,
    int *shape); // if indices overflow then return 1 else return 0
...
Tensor *einsum(int num_idx, int *idxs1, Tensor *m1, int *idxs2, Tensor *m2,
               int dim, int *idxs);
Tensor *einsum2(char *idxs, Tensor *m1,
                Tensor *m2); // Wrapper over einsum for easy calling
